...
function buildUrls(colln){
  var urlsForJS = "module.exports = Object.freeze({\n";
  if(colln.item){
    urlsForJS += findNestedRoutes(colln.item)
  }
  urlsForJS += "});"
  // console.log(urlsForJS)
  fileHandlers.createFileAndWrite("urls.js", "./", urlsForJS);
}
...
function findNestedRoutes(colln){  
  ...
  for(let i=0; i< colln.length; i++){
    let c = colln[i]
    if(c.item){
      urls += `${getCamelcase(c.name)} : {\n${findNestedRoutes(c.item)}\n},\n`
    }else{
      urls += getCamelcase(c.name) + ":" + `\"${formatUrl(c.request.url.raw)}\",\n`
...
function formatUrl(url){
  while (url.includes("{{") || url.includes("}}")) {
    ...
    url = url.substr(0, fIndex) +
    url.substr(lIndex + 2, url.length);
  }
